...
def  main():
  ...
  installer = "\installer\IPlugWebUIe_SK Installer.exe"
   
  if demo:
    installer = "\installer\IPlugWebUIe_SK Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugWebUIe_SK manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugWebUIe_SK-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugWebUIe_SK-v" + FULLVERSIONSTR + "-win-demo.zip"
