...
def  main():
  ...
  installer = "\installer\IPlugWebUI_SK Installer.exe"
   
  if demo:
    installer = "\installer\IPlugWebUI_SK Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugWebUI_SK manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugWebUI_SK-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugWebUI_SK-v" + FULLVERSIONSTR + "-win-demo.zip"
