window.__sk_iPlug2 = {
  ...
  isInitialized: (timeout = 3000) => {
    return new Promise((resolve, reject) => {
      ...
      var await_sk_iPlug2_timer = setInterval(() => {

        var end_ms = Date.now()
        var elapsed_ms = end_ms - start_ms
        if (elapsed_ms > timeout) {
          clearInterval(await_iPlug2_timer)
          var err = "[SK_iPlug2 Core] SK_iPlug2 core was never initialized in time"
          console.error(err)
          return reject(err)
        }
        ...
        if (!SK_iPlug2) return
        if (!SK_iPlug2.initialized) return

        clearInterval(await_iPlug2_timer)
        console.log("[SK_iPlug2 Core] Initialized")
        resolve()
      }, 1)
    })
  }
...
class SK_iPlug2_IPC {
  ...
  sendToBE(event_id, data = {}, type = "request", overridePacketInfo = {}) {
    if (!event_id) throw "[SK_iPlug2 IPC.sendToBE] Event ID cannot be empty"
  ...
  request(event_id, data = {}, timeout = 5000) {
    return new Promise((resolve, reject) => {
      ...
      var timeoutTimer = setTimeout(() => {
        var err = `[SK_iPlug2 IPC.send] Send event ${event_id} timed out`
        console.error(err)
        reject(err)
      }, timeout)
  ...
  message(data) {
    this.sendToBE("SK_iPlug2_IPC_Message", data, "message")
  }

  on(event_id, cb, once) {
    if (!event_id) throw "[SK_iPlug2 IPC.on] Event ID cannot be empty"
  ...
  handleIncoming(res) {
    ...
    else throw `[SK_iPlug2 IPC.handleIncoming] Invalid incoming IPC message type`
  }
  ...
  handleResponse(res) {
    ...
    if (!awaiter) throw `[SK_iPlug2 IPC.handleResponse] Invalid IPC response for event ID "${res.event_id}" with message ID "${res.msg_id}"`
...
class SK_iPlug2_Core {
  ...
  init() {
    this.ipc = new SK_iPlug2_IPC({
      sender_id: this.id
    })

    this.initialized = true

    console.log("[SK_iPlug2] Core successfully initialized")
  }
}

window.SK_iPlug2 = new SK_iPlug2_Core({ id: Date.now().toString() })
window.skip2 = SK_iPlug2 //short name